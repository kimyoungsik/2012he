...
//    private ISenderAdapter iSenderA;
    ...
    public MainView(WindowListener wl, IFileActionAdapter fileActionA, IEditActionAdapter editActionA, ITESModelAdapter modelA, IServerAdapter iSerA) {
        super("DEAF Editor 1.0.2");
            ...
            menuBar.setPreferredSize(new Dimension(1040, 25));
            {
                ...
                menuBar.add(editMenu);
                {
                    ...
                    fontMenuItem.addActionListener(new ActionListener() {
                        ...
                        public void actionPerformed(ActionEvent arg0) {
                            ...
                            FontTracker ft = new FontTracker(textPane.getFont(), textPane.getForeground(), textPane.getBackground());
...

                {
                    ...
                    serverNetworkMenuItem.addActionListener(new ActionListener() {
                        ...
                        public void actionPerformed(ActionEvent event) {
                            Network_Server dialog = new Network_Server(iSerA);
                ...
                {//오른쪽 축약 테이블
...

                    {
                        ...
                        newRepositoryBtn.setIcon(new javax.swing.ImageIcon("img/new.png"));
                        ...
                        openRepositoryBtn.setIcon(new javax.swing.ImageIcon("img/open.png"));
                        openRepositoryBtn.setToolTipText("라이브러리 창을 불어옵니다.");
                        openRepositoryBtn.addActionListener(new ActionListener() {

                            public void actionPerformed(ActionEvent e) {
                                // open file loader which has a default folder of src/doc
                                OpenRepoDialog dialog = new OpenRepoDialog(MainView.this);
                                dialog.setVisible(true);
                    ...
                    }
                    {
                        ...
                        //splitPane.add(reposScrollPane, JSplitPane.RIGHT);
                    }
                }
            }
    ...
    public void keyTyped(KeyEvent e) {
        ...
        if (converters.contains(key)) {//변환글자와 같은것이 있으면
            ...
            for (int i = 0; i < reposTabbedPane.getComponentCount(); i++) {
                ...
                if ((to != null) && (!to.isEmpty())) { //careful with nullPointerError
                    StringBuilder tmpBuffer = new StringBuilder(textPane.getText());
                    ...
                    textPane.setCaretPosition(pos + (to.length() - from.length()));//+ to.length()-1
                    //늘어난 길이만큼 캐럿 옮기기. 현재위치 +(바뀔글자길이 - 축약글자길이)


                    //break를 없앰으로써 가장 최근의 것을 사용하는 로직//하지만 break 없애면 오류발
                    break;

                    //지금여기는 바로 ; 를 하면 바로 변환하게 되어있다.
                    //여러가지 모드로 바꾸도록 수정 
                }
            }
        }
    ...
    public void keyReleased(KeyEvent e) {
        ...
        if (e.getKeyCode() == KeyEvent.VK_F2) {
            ...
            AddAbbrShortcut addAbbr = new AddAbbrShortcut((RepoView) reposTabbedPane.getSelectedComponent(), getInputWord(pos));
