 ...
 * 수정자 : 김영식
 * 수정사항
 * 1. client가 받는 텍스트뷰를 설정하여 탭으로 나누어 놓음
 * showTextPane
 * 2. 그 부분은 사용자가 수정 불가능 한 부분으로 화면이 찰경우 위로 올라감.
 * limitedLin
 * 
 * 
 *
...
public class MainView extends JFrame implements KeyListener {
	...
	/**
	 * Generated serial version ID for MainView class
	 */
...
//					});
					...
					exitMenuItem.addActionListener(new ActionListener() {
						...
						public void actionPerformed(ActionEvent evt) {
							// System.out.println("exitMenuItem.actionPerformed, event="
							// + evt);
...
//					editMenu.add(new JSeparator());
					...
					fontMenuItem.addActionListener(new ActionListener() {
						...
						public void actionPerformed(ActionEvent arg0) {
							int selectedIndex = textTabbedPane
									.getSelectedIndex();
							FontChooserView fcv = new FontChooserView(
									MainView.this);
							FontTracker ft = null;
							if (selectedIndex == 0) {
								ft = new FontTracker(showTextPane.getFont(),
										showTextPane.getForeground(),
										showTextPane.getBackground());
							} else if (selectedIndex == 1) {
								ft = new FontTracker(textPane.getFont(),
...

				{
					...
					clientNetworkMenuItem.setAccelerator(KeyStroke
							.getKeyStroke(KeyEvent.VK_I, Toolkit
									.getDefaultToolkit()
									.getMenuShortcutKeyMask()));

					clientNetworkMenuItem
							.addActionListener(new ActionListener() {

								public void actionPerformed(ActionEvent event) {
									Network_Client dialog = new Network_Client(
											iCliA);
					...
					resizeNetworkMenuItem.setAccelerator(KeyStroke
							.getKeyStroke(KeyEvent.VK_R, Toolkit
									.getDefaultToolkit()
									.getMenuShortcutKeyMask()));

					resizeNetworkMenuItem
							.addActionListener(new ActionListener() {

								public void actionPerformed(ActionEvent event) {
									// 크기 조정
									Dimension showText = textTabbedPane
											.getSize();
									showTextheight = showText.getHeight();
									limitedLine = (int) showTextheight
											/ (currentFontHight + 15);
...
//					allTextNetworkMenuItem.setAccelerator(KeyStroke
//							.getKeyStroke(KeyEvent.VK_T, Toolkit
//									.getDefaultToolkit()
//									.getMenuShortcutKeyMask()));
//
//					allTextNetworkMenuItem
//							.addActionListener(new ActionListener() {
//
//								public void actionPerformed(ActionEvent event) {
//									String str = ReceiverThread.allString;
//									// String receiverStr =
//									// iReceiverA.GetAllText();
...
//							});
					...
					manualHelpMenuItem.addActionListener(new ActionListener() {
						...
						public void actionPerformed(ActionEvent event) {
							try {
								...
								System.out
										.println("Opening manual was unsuccessful");
								e.printStackTrace();
							}
						}
					});
				...
				{// 왼쪽 텍스트
					...
					JScrollPane showTextScrollPane = new JScrollPane(
							showTextPane);
					...
					limitedLine = (int) this.showTextheight
							/ (this.currentFontHight + 15);
	...
	/**
	 * Handle the key typed event from the text field. Takes a key event of
	 * converters to convert abbreviations to original texts.
	 */
	...
	public void keyReleased(KeyEvent e) {
		...
		if (e.getKeyCode() == KeyEvent.VK_ENTER) {
			// textPane.scrollRectToVisible(new
			// Rectangle(0,textPane.getHeight()+100,1,1));
			String alltext = textPane.getText();
			if (alltext.endsWith("\n\n")
					&& (textPane.getCaretPosition() >= alltext.length())) {
				// String new_line = (autoTimingSelected) ?
				// timer.elapsedAsStringShort() + " " + NEW_SPEAKER :
				// NEW_SPEAKER;
				// 잘 안되고 있는듯
				// 아래로 바꾸면 될듯
				// String new_line =
	...
	/**
	 * If returning object is not null, createRepo() was successful; otherwise,
	 * unsuccessful.
	 * 
	 * @param title
	 * @param source
	...
	public void setTextFontTracker(FontTracker newFontTracker) {
		...
		this.limitedLine = (int) this.showTextheight
				/ (this.currentFontHight + 15);
	}
	...
	public void readKey() throws IOException {
		...
		try {
			...
			FileReader fr = new FileReader(System.getProperty("user.home")
					+ File.separator + "Library/deafsoftwarec.txt");
//			do {
//				ch = fr.read();
//				if (ch != -1) {
//					data += ((char) ch);
